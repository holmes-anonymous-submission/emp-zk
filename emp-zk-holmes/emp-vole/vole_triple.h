...
public:
	...
	~VoleTriple() {
		std::cout << "Total triples generated = " << total_triple_generated << std::endl;
	...
	void extend(__uint128_t *buffer) {
		...
		total_triple_generated += ot_limit;
	}
	...
	void check_triple(__uint128_t x, __uint128_t* y, int size) {
		if(party == ALICE) {
			...
			for(int i = 0; i < size; ++i) {
				__uint128_t tmp = mod_uint128(delta*(y[i]>>64));
				tmp = mod_uint128(tmp+k[i]);
