...
public:
	...
	__uint128_t extend() {
		...
		for(int i = 0; i < m; ++i) {
			ch[1] = v[i];
			v[i] = mod_uint128(w[i]+ch[delta_bool[i]]);
		}
	...
	__uint128_t extend(__uint128_t u) {
		...
		for(int i = 0; i < m; ++i) {
			...
			extract_fp(w0[i]);
			extract_fp(w1[i]);
			w1[i] = mod_uint128(w1[i]+u);
			w1[i] = pr - w1[i];
			tau[i] = mod_uint128(w0[i]+w1[i]);
		}
	...
	__uint128_t prm2pr(__uint128_t *a) {
		...
		__uint128_t tmp;
		for(int i = 0; i < m; ++i) {
			tmp = mod_uint128(a[i]<<i);
			ret = mod_uint128(ret+tmp);
		}
		return ret;
	}
	...
	void prm2pr(__uint128_t *ret, __uint128_t *a, int size) {
		...
		for(int i = 0; i < m; ++i) {
			for(int j = 0; j < size; ++j) {
				tmp = mod_uint128(a[i*size+j]<<i);
				ret[j] = mod_uint128(ret[j] + tmp);
			}
		}
	}
	...
	void check_triple(uint64_t *a, __uint128_t *b, int sz) {
		if(party == ALICE) {
			...
			for(int i = 0; i < sz; ++i) {
				__uint128_t tmp = mod_uint128((__uint128_t)a[i]*delta);
				tmp = mod_uint128(tmp + c[i]);
				if(tmp != b[i]) {
